...
public class SMSAddActivity extends Activity implements OnClickListener,
		SMS4UConstants {

	private Button submitSMS, cancelSMS;
	private ImageButton addContacts, datePicker, timePicker;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		datePicker = (ImageButton) findViewById(R.id.dateButton);
		timePicker = (ImageButton) findViewById(R.id.timeButton);
		submitSMS = (Button) findViewById(R.id.submit_sms_button);
		smsTitle = (EditText) findViewById(R.id.sms_title);
		addContacts = (ImageButton) findViewById(R.id.add_contacts);
		smsMessage = (EditText) findViewById(R.id.sms_message);
		smsNumbers = (EditText) findViewById(R.id.sms_numbers);
		cancelSMS = (Button) findViewById(R.id.cancel_sms_button);
		...
		// setImageBasedOnScreen(addContacts);
	}
	...
	public void onClick(View view) {
		...
		if (view == submitSMS) {
			if (validateInputs(smsNumbers, smsTitle, dateTextView,
				...
				SMSDataModel model = new SMSDataModel();
				model.setTitle(smsTitle.getText().toString());
				model.setNumbersForSMS(smsNumbers.getText().toString());

				model.setDateAndTime(prepareDate(dateTextView.getText()
						.toString(), timeTextView.getText().toString()));
				model.setMessage(smsMessage.getText().toString());
				...
				if (contacts != null) {
					...
					if (smsNumbers.getText() != null
						...
						if (!currentNumbers.endsWith(COMMA_CHARACTER)) {
							if (currentNumbers != null && currentNumbers != ""
	...
	private boolean validateInputs(EditText smsNumbers, EditText smsTitle,
		...
		if (date.getText() == null || date.getText().toString() == null
			...
			Toast.makeText(this, "Provide date for message.", Toast.LENGTH_LONG).show();
			return false;
		}
		if (time.getText() == null || time.getText().toString() == null
				|| time.getText().equals("")
				|| time.getText().toString().length() == 0) {
			Toast.makeText(this, "Provide time for message.", Toast.LENGTH_LONG).show();
			return false;
		}
		if (smsMessage.getText() == null
			...
			Toast.makeText(this, "Provide Message body.", Toast.LENGTH_LONG).show();
			return false;
		}
